...
def all_students_tuple_list(filename):
    ...
    #loop through each line
        ...
        #strip white space
            line = line.rstrip()
        #split at | and store at word var
            columns = line.split("|")
        #unpack columns: first, last, house, advisor, cohort = line.split("|")
            first, last, house, advisor, cohort = line.split("|")
        #create condition to exclude data if advisor == ""
            if house != "":
                student_info_tup = (first + " " + last, house, advisor, cohort)        
                #.add tuple to student_list
                student_list.append(student_info_tup)     
...
def find_cohort_by_student_name(student_list):
    ...
    """
    #print all_students_data
    #make while loop that goes through all tuples
    #while True:
    name_list = []
    for tup in all_students_data:
        name_list.append(tup[0])
    for tup in all_students_data:
    #prompt user input
        #print all_students_data
        student_name = raw_input("Who are you looking for? ")
    # if user input == "exit" then break
        if student_name == "exit":
            break
    #elif student_name not in all_students_data print "Student not found." - continue
        elif student_name in name_list: 
        #print '{} was in the {} cohort.'
            if student_name in tup:
                print student_name, " was in the ",tup[3]
                # print "{} was in the {} cohort.".format(tup[0],tup[3])
